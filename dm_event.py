...
class dm_event(object):
    def __init__(self, N_p_Si, N_n_Si, mass_dm, cross_section, mass_det, t_exp, detection, Eemin=4e-4,Eemax=7, sigma_res_Ee = 4e-4, sigma_Ee = 4e-4, eff = 1):
    ...
    def simul_ev(self, *bkg_pars, **kwargs):
        ...
        self.signal_ev = random_signal_det(self.n_s_det, self.N_p_Si, self.N_n_Si, self.mass_dm, self.cross_section, self.C_sig, self.detection, Eemin = self.Eemin, Eemax = self.Eemax, sigma_res_Ee = self.sigma_res_Ee, sigma_Ee = self.sigma_Ee, step = step) 
        #if self.detection:
        #    ### Events in Eee units
    ...
    def likelihood(self, deltaLL = False, errors = False, **kwargs):
        ...
        if deltaLL:
            ...
            self.lnL_sb = log_likelihood(self.theta, lindhard_inv(self.events), self.Ermin, self.Ermax, self.N_p_Si, self.N_n_Si, self.mass_dm, self.mass_det*self.t_exp, self.detection, bkg, sigma_Ee, sigma_Ee_b, sigma_res_Ee, self.eff)
            ### Background only likelihood
            b_fit = minimize(log_likelihood_bkg, x0[1], bounds=[bnds[1]], method='L-BFGS-B', args = (lindhard_inv(self.events), self.Ermin, self.Ermax, sigma_Ee_b, sigma_res_Ee, self.eff)).x
            self.lnL_b = log_likelihood_bkg(b_fit, self.events, self.Eemin, self.Eemax, sigma_Ee_b, sigma_res_Ee, self.eff)
    ...
    def upper_limit(self, deltaLL = 4.5, cf = 0.95, plot = False, **kwargs):
        ...
        lnL_lambda = lambda xs:  -log_likelihood([xs, theta_max[1]], Er, self.Ermin, self.Ermax, self.N_p_Si, self.N_n_Si, self.mass_dm, self.mass_det*self.t_exp, self.detection, True, self.sigma_Ee, self.sigma_Ee_b, self.sigma_res_Ee, self.eff)
        ...
        def lnL_prob(xs_upper):
            ...
            if bnds[0][0] == xs_upper:
                ...
                return cf - 1  
            xs_range = np.geomspace(bnds[0][0], xs_upper, 100)
        ...
        self.cross_section_95 = bisect(lnL_prob, bnds[0][0], bnds[0][1], rtol=1e-3)
        ...
        if plot:
            ...
            #plt.xscale("log")
            plt.show()
        #self.cross_section_95 = bisect(lnL_prob, bnds[0][0], bnds[0][1], rtol=1e-1)
        #print(self.cross_section_95)
        ...
        #print("Max Cross: ", theta_max[0])

        ### Upper limit cross section
        #self.cross_section_95 = bisect(lnL_func, theta_max[0], bnds[0][1]) 
